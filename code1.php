...
if (isset($_POST['save-btn'])) {
    ...
    $email_query = "SELECT * FROM etudiant WHERE username='$username' AND password='$password' AND status='$status' AND gender='$gender' AND idN='$niveau'";
    ...
    if (mysqli_num_rows($email_query_run) > 0) {
        ...
        // Prepare the query
        $query = "INSERT INTO etudiant (username, password, status, gender, idN) VALUES (?, ?, ?, ?, ?)";
        ...
        if ($stmt->execute()) {
            echo "<script>alert('Etudiant Profile Added');</script>";
            header('Location: tableE.php');
            exit();
        } else {
            echo "<script>alert('Etudiant Profile Not Added: " . $stmt->error . "');</script>";
        }
    }
} else {
