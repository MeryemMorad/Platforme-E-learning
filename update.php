...
// Vérifiez que id_EN est défini et est un entier
$id_user = isset($_GET['id_EN']) ? intval($_GET['id_EN']) : 0;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "UPDATE `enseignant` SET `username`='$username', `password`='$password', `status`='$status', `gender`='$gender', `id_M`='$matiere', `idN`='$niveau' WHERE `id_EN`=$id_user";
    $result = mysqli_query($conn, $sql);

    if ($result) {
        header("Location: ENtable.php?msg=Update successful");
        exit();
    } else {
...
$sql = "SELECT * FROM `enseignant` WHERE `id_EN` = $id_user LIMIT 1";
$result = mysqli_query($conn, $sql);
...
if (!$result_matiere) {
    die('Erreur de requête pour matières : ' . mysqli_error($conn));
}
?>
    ...
    <div class="main-content">
        ...
        <div class="container">
            <div class="text-center mb-4">
                <h3>Edit Enseignant Information</h3>
                ...
                <form action="" method="post" style="width: 50vw; min-width: 300px;">
                    ...
                    <div class="mb-3">
                        ...
                        <?php while ($row_matiere = mysqli_fetch_assoc($result_matiere)) { ?>
                                <option value="<?php echo $row_matiere['id']; ?>" <?php echo ($row_matiere['id'] == $row['id_M']) ? 'selected' : ''; ?>>
