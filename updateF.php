...
$id_user = isset($_GET['id_F']) ? intval($_GET['id_F']) : 0;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "UPDATE `facilitateur` SET `username`='$username', `password`='$password', `status`='$status', `gender`='$gender', `idN`='$niveau' WHERE `id_F`=$id_user";
    $result = mysqli_query($conn, $sql);

    if ($result) {
        header("Location: tableF.php?msg=Update successful");
        exit();
    } else {
...
$sql = "SELECT * FROM `facilitateur` WHERE `id_F` = $id_user LIMIT 1";
$result = mysqli_query($conn, $sql);
...
<body>
    ...
    <div class="container">
        <div class="text-center mb-4">
            <h3>Edit Facilitateur Information</h3>
